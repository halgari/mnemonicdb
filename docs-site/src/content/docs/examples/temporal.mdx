...
-- Create person attributes using the admin view
...
-- Transaction 1: Promote Alice to Senior Developer
CALL mnemonic_begin_tx();
UPDATE persons SET title = 'Senior Developer' WHERE name = 'Alice';
CALL mnemonic_commit_tx();

-- Transaction 2: Add another person
CALL mnemonic_begin_tx();
INSERT INTO persons (name, title) VALUES ('Bob', 'Manager');
CALL mnemonic_commit_tx();

-- Transaction 3: Promote Alice again
CALL mnemonic_begin_tx();
